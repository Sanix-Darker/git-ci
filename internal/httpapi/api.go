...
func (a *API) routes() http.Handler {
	...
	mux.Handle("GET /app/runs/{run}/artifacts/{artifact}", a.requireWebAuth(http.HandlerFunc(a.handleArtifactDownload)))
	...
	mux.Handle("GET /api/v1/runs/{run}/artifacts", a.requireAuth(http.HandlerFunc(a.handleRunArtifacts)))
	mux.Handle("GET /api/v1/runs/{run}/artifacts/{artifact}", a.requireAuth(http.HandlerFunc(a.handleArtifactDownload)))
	mux.Handle("GET /api/v1/runs/{run}/test-reports", a.requireAuth(http.HandlerFunc(a.handleRunTestReports)))
	mux.Handle("GET /api/v1/projects/{project}/caches", a.requireAuth(http.HandlerFunc(a.handleProjectCaches)))
...
func (a *API) handleAPIRoot(writer http.ResponseWriter, _ *http.Request) {
	writeJSON(writer, http.StatusOK, map[string]any{
		"api":          "v1",
		"capabilities": []string{"auth", "local-projects", "workflow-discovery", "durable-runs", "local-worker", "runner-inventory", "runner-matching", "step-summaries", "protected-environments", "approvals", "environment-secrets", "deployments", "rollback", "job-replay", "step-replay", "audit"},
	})
}
