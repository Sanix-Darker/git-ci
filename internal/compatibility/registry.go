...
var entries = []Entry{
	...
	feature("shared-webhooks", "shared", "triggers", "Signed provider webhooks", StatePartial, "GitHub, GitLab, and generic signed endpoints apply workflow admission and enqueue deduplicated push or pull-request runs.", "The full GitHub event and GitLab pipeline-source matrices are not implemented.", "provider payload + local diff + API + Chromium", ""),
	...
	feature("shared-release-objects", "shared", "delivery", "Release objects", StateSupported, "Create, edit, publish, filter, and inspect SQLite-backed releases from successful runs and immutable local Git tags, with linked artifacts and deployments.", "Provider-side release publication remains a separate credential-bound capability.", "store + API + Git fixtures + responsive Chromium", ""),
	...
	feature("github-pull-request", "github", "triggers", "Pull request events", StatePartial, "Parse pull_request policies, normalize signed deliveries, apply target branch and activity filters, and derive three-dot path changes from the local checkout.", "Untrusted fork fetching and GitHub merge-ref emulation require an explicit operator trust policy.", "parser + admission + local Git diff + Chromium", "https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#pull_request"),
	...
	feature("github-continue-on-error", "github", "execution", "Expression-aware continue-on-error", StateSupported, "Evaluate job allowed-failure expressions per immutable matrix cell while keeping failed jobs visible.", "Expressions are limited to gci's deterministic condition subset and immutable matrix context.", "parser + matrix evaluator + API + responsive Chromium", "https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax#jobsjob_idcontinue-on-error"),
	feature("github-workflow-run", "github", "triggers", "Completed workflow chaining", StatePartial, "Dispatch durable downstream workflows from terminal local runs with exact ref/SHA provenance, conclusion context, restart-safe deduplication, and a three-level chain cap.", "gci currently emits the completed activity only, pins the source commit instead of GitHub's default branch, and keeps the normal local secret policy without privilege escalation.", "trigger parser + SQLite outbox + manager restart/depth + API + responsive Chromium", "https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#workflow_run"),
	...
	feature("github-artifacts-cache", "github", "artifacts", "Artifacts, cache, and test reports", StateSupported, "Store checksummed project-scoped artifacts, isolated caches, cache-hit outputs, split restore/save cache actions, and JUnit report metadata.", "", "artifact/cache/report integration", "https://docs.github.com/en/actions/reference/workflows-and-actions/dependency-caching"),
	...
	feature("gitlab-needs-optional", "gitlab", "graph", "Optional needs", StateSupported, "Preserve optional dependency edges, prune absent targets, and admit downstream jobs when optional targets are skipped by rules.", "", "parser + matrix normalization + manager + responsive Chromium", "https://docs.gitlab.com/ci/yaml/#needsoptional"),
	...
	feature("gitlab-rules", "gitlab", "triggers", "workflow and job rules", StatePartial, "Evaluate ordered if, RE2 regex, changes, exists, variables, when, and allow_failure contracts.", "Every predefined variable and the newer Ruby-regex changes/exists dialects are not supported.", "condition engine + rules fixtures + Chromium E2E", "https://docs.gitlab.com/ci/yaml/#rules"),
	...
	feature("gitlab-retry", "gitlab", "execution", "Automatic retry", StateSupported, "Retry failed jobs in place with durable attempt history, failure selectors, and exit-code selectors.", "GitLab's maximum of two retries is enforced; attempts reuse the same commit workspace while runtime containers are recreated.", "parser + SQLite attempt ledger + manager + Chromium E2E", "https://docs.gitlab.com/ci/yaml/#retry"),
	feature("gitlab-allow-failure-exit-codes", "gitlab", "execution", "Exit-code allowed failures", StateSupported, "Keep jobs visibly failed while allowing downstream execution and successful pipeline completion only when the final attempt matches configured exit codes.", "", "parser + immutable semantics + SQLite attempt ledger + responsive Chromium", "https://docs.gitlab.com/ci/yaml/#allow_failureexit_codes"),
	...
	feature("gitlab-child-pipelines", "gitlab", "reuse", "Child and multi-project pipelines", StatePartial, "Dispatch repository-contained child configurations as immutable same-commit runs with async or mirrored bridge status, nested lineage, cancellation, API links, and expandable DAG cards.", "Multi-project, dynamic artifact, composed include, and cross-pipeline artifact contracts remain planned.", "parser + SQLite lineage + manager + API + Chromium E2E", "https://docs.gitlab.com/ci/pipelines/downstream_pipelines/"),
