...
var entries = []Entry{
	...
	feature("shared-audit", "shared", "operations", "Audit trail", StatePartial, "Mutating service operations record durable actor and resource events.", "Audit is API-backed but does not yet have a dedicated searchable web ledger.", "store + HTTP", ""),
	...
	feature("gitlab-manual-jobs", "gitlab", "execution", "Manual jobs", StatePlanned, "Pause a running pipeline at when: manual and expose a play control.", "Current play controls create explicit replay runs after terminal execution.", "roadmap", "https://docs.gitlab.com/ci/jobs/job_control/#create-a-job-that-must-be-run-manually"),
